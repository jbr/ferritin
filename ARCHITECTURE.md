...
- **`mcp`** (off by default, implies `serve`) adds a stateless [Model Context Protocol](https://modelcontextprotocol.io) endpoint at `/mcp` — a sibling to `/api`, not nested under it. It implements the **Streamable HTTP** transport in its simplest conformant form: JSON-only, no SSE and no sessions. A `POST` carrying one JSON-RPC *request* gets a single `application/json` response; a *notification* gets `202 Accepted` with no body; a `GET` gets `405` (the server offers no server-initiated stream). Two tools mirror the JSON API — `get` (item by path) and `search` (a *single* crate, deliberately: cross-crate search over the public corpus is infeasible) — and each returns the **agent** render of the very `Document` the CLI produces, which is exactly the token-efficient Markdown an LLM tool result wants. Messages are decoded and executed on the same big-stack rayon worker as the JSON API, since the documentation lookups recurse identically and would overflow an async worker's stack. The endpoint has its **own** rate limiter (`FERRITIN_MCP_RATELIMIT`), separate from `/api`'s, so agent traffic — few, expensive, non-browser calls — is tuned on its own quota.

  The MCP protocol types (`serve::mcp::protocol`) are a **trimmed copy** of the [`mcplease`](https://github.com/jbr/mcplease) crate's transport-agnostic core — its JSON-RPC message shapes plus the `Tool`/`AsToolsList` schema machinery, with the stdio transport, clap-based `tools!` macro, and session store left behind. Copying rather than depending keeps the feature's dependency delta to `serde_json` (already present transitively via `schemars`, which the schema generation is built on) plus `schemars` itself; a `mcplease` dependency would instead drag in clap, notify, dirs, env_logger and shellexpand, at odds with the server features' deliberate delta minimalism. `mcplease::McpRequest::execute` — the piece that maps a decoded `initialize`/`tools/list`/`tools/call` request to a response with no I/O of its own — is what makes the HTTP glue in `serve::mcp` thin. Each `tools/call` builds its own short-lived `Navigator` over the shared `Store`, pinning only what the lookup touches, exactly as the JSON API handlers do.
...
One serve-mode endpoint stands apart from the Store/Navigator machinery: `/api/typeahead` (crate-name completion) is answered by `TypeaheadService`, which merges the standard library crates (absent from crates.io, present on this server) into queries against the shared [`CrateIndex`](#the-crates-io-namespace-as-an-artifact-crate_names) and hoists an exact match to the front. The matching itself (name-prefix ∪ interior-token ∪ stemmed description, additive multi-word scoring, popularity ranking) lives in `CrateIndex::typeahead` — see [Crate-name typeahead](#crate-name-typeahead) and [Description matching](#description-matching).
